...
const WorkflowSelect = () => {
    ...
    const filteredWorkflows = dataApprovalWorkflows.filter(
        (workflow) => workflow.id === 'stfYnMHvmta'
    )

    return (
        <ContextSelect
            dataTest="workflow-context-select"
            prefix={i18n.t('Dataset')} // name for the select option
            placeholder={i18n.t('Choose a Dataset')}
            value={value}
            open={open}
